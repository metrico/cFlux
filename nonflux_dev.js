			...
			parsed.returnColumns.forEach(function(source,i){
			  source.sourceColumns.forEach(function(metric_id){
				if (metric_id.value){
				   ...
				   inner.push("SELECT toStartOfMinute(toDateTime(timestamp_ms/1000)) as minute, name, avg(value) as mean, labelname, labelvalue"
					...
					+" ANY INNER JOIN ("
						...
						+" FROM ("
							...
							+" PREWHERE measurement='hepic_statistics_registrations'"
							+" AND name IN ('unauth') AND hasAny(['gid'], labelname) = 1"
							if (filters.length > 0) filters.forEach(function(filter){
								+" AND labelvalue[arrayFirstIndex(x -> (x = filter.name), labelname)] = filter.value)"
							}
						+" WHERE labelname='captid' )"
					+" USING(fingerprint)"
